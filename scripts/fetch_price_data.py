...
def fetch_yfinance_data(symbol: str, start_date: str | None = None, end_date: str | None = None, period: str | None = None, interval: str = "1d") -> pd.DataFrame:
    ...
    try:
        ...
        if 'Datetime' in history.columns:
            ...
            logger.error(f"Timestamp column ('Datetime' or 'Date') not found in yfinance data for {symbol}.")
            return pd.DataFrame()

        history[ts_column] = pd.to_datetime(history[ts_column], utc=True) # Ensure datetime objects, assume UTC if mixed/naive
        history[ts_column] = history[ts_column].dt.tz_localize(None) # Make naive for DB storage (as UTC)
...
def save_data_to_db(session, data: pd.DataFrame, asset_obj: Asset, source_name: str, dry_run: bool = False):
    ...
    if asset_obj is None:
        logger.error(f"Invalid asset object (None) provided. Cannot save price data for source {source_name}.")
        return
    ...
    if not dry_run and (asset_obj.id is None or asset_obj.id == -1):
        logger.error(f"Invalid asset ID for asset '{asset_obj.symbol}'. Cannot save price data.")
        return
    ...
    for _, row in data.iterrows():
        timestamp_val = pd.to_datetime(row[ts_column]) # Should already be datetime from fetch

        if dry_run:
            logger.info(f"[DRY RUN] Would add PriceData: Asset Symbol {asset_obj.symbol}, Timestamp {timestamp_val}, "
        ...
        try:
            existing_entry = session.query(PriceData).filter_by(
                asset_id=asset_obj.id,
                timestamp=timestamp_val,
                source=source_name
            ).first()
        except Exception as e_query:
            logger.error(f"Error querying existing PriceData for {asset_obj.symbol} at {timestamp_val}: {e_query}", exc_info=True)
            error_count += 1
        ...
        try:
            ...
            session.rollback()
            logger.debug(f"Integrity error (likely duplicate despite check) for asset ID {asset_obj.id} at {timestamp_val} from {source_name}. Skipping.")
            skipped_count += 1
        except Exception as e:
            session.rollback()
            logger.error(f"Error saving data for asset ID {asset_obj.id} at {timestamp_val} from {source_name}: {e}", exc_info=True)
            error_count +=1

    log_prefix = "[DRY RUN] " if dry_run else ""
    logger.info(f"{log_prefix}Data saving for asset '{asset_obj.symbol}' from {source_name} complete. "

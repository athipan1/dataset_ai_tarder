...
from ai_trader.db.session import \
    SessionLocal  # SessionLocal is a configured sessionmaker
...
def save_data_to_db(
    ...
    """
    if asset_obj is None or (
        not dry_run and (asset_obj.id is None or asset_obj.id == -1)
    ):
        logger.error(
            f"Invalid asset object provided for source {source_name}. Cannot save price data."
        )
        return
    ...
    data_to_insert["asset_id"] = asset_obj.id
    data_to_insert["source"] = source_name
    ...
    if dry_run:
        logger.info(
            f"[DRY RUN] Would insert {len(records)} price data records for asset '{asset_obj.symbol}'."
        )
        if records:
    ...
    try:
        ...
        stmt = stmt.on_conflict_do_nothing(
            index_elements=["asset_id", "timestamp", "source"]
        )
        result = session.execute(stmt)
        session.commit()
        logger.info(
            f"Data saving for asset '{asset_obj.symbol}' from {source_name} complete. "
            f"Inserted: {result.rowcount} new records."
        )
    except Exception as e:
        session.rollback()
        logger.error(
            f"Error during bulk saving of price data for asset '{asset_obj.symbol}': {e}",
            exc_info=True,
        )

...
def fetch_yfinance_data(symbol: str, start_date: str | None = None, end_date: str | None = None, period: str | None = None, interval: str = "1d") -> pd.DataFrame:
    """
    ...
    if start_date and end_date:
        try:
            ...
            log_msg_parts.append(f"start: {start_date}, end: {end_date} (querying up to {actual_yf_end_date})")
        except ValueError:
            logger.error(f"Invalid date format for start_date ('{start_date}') or end_date ('{end_date}'). Use YYYY-MM-DD.")
            return pd.DataFrame()
    elif period:
        ...
        logger.error("Either start_date/end_date or period must be provided for yfinance.")
        return pd.DataFrame()
    ...
    try:
        ...
        if history.empty:
            logger.warning(f"No data found for {symbol} with specified parameters from Yahoo Finance.")
            return pd.DataFrame()
        ...
        ts_column = 'Datetime'
        if ts_column not in history.columns:
            logger.error(f"Timestamp column ('Datetime') not found in yfinance data for {symbol}.")
            return pd.DataFrame()

        history[ts_column] = pd.to_datetime(history[ts_column], utc=True)
        ...
        if start_date and end_date: # only if end_date was originally specified
            ...
            requested_end_datetime = pd.to_datetime(end_date).replace(hour=23, minute=59, second=59, microsecond=999999)
        ...
        logger.error(f"Error fetching data for {symbol} from Yahoo Finance: {e}", exc_info=True)
        return pd.DataFrame()


def fetch_binance_data(symbol: str, start_date: str | None = None, end_date: str | None = None, period: str | None = None, interval: str = "1d") -> pd.DataFrame:
    """
    ...
    try:
        ...
        if api_key and api_secret:
            ...
            logger.debug("Attempting public Binance API access (no key found in settings).")
    except Exception as e:
    ...
    if start_date:
        ...
        if end_date:
            try:
                ...
                log_msg_parts.append(f"end: {end_date} (querying up to {actual_binance_end_str})")
            except ValueError:
                logger.error(f"Invalid date format for end_date ('{end_date}'). Use YYYY-MM-DD.")
                return pd.DataFrame()
        else:
        ...
        if period.endswith("y"): start_dt_calc = current_time_utc - timedelta(days=365 * int(period[:-1]))
        elif period.endswith("M"): start_dt_calc = current_time_utc - timedelta(days=30 * int(period[:-1])) # Approx
        elif period.endswith("w"): start_dt_calc = current_time_utc - timedelta(days=7 * int(period[:-1]))
        elif period.endswith("d"): start_dt_calc = current_time_utc - timedelta(days=int(period[:-1]))
        elif period.endswith("h"): start_dt_calc = current_time_utc - timedelta(hours=int(period[:-1]))
        elif period == "max":
            start_dt_calc = datetime(2017, 1, 1, tzinfo=timezone.utc) # Common practical limit
        ...
        logger.error("Either start_date/end_date or period must be provided for Binance.")
        return pd.DataFrame()
    ...
    try:
        ...
        if not klines:
            logger.warning(f"No data found for {symbol} with specified parameters from Binance.")
            return pd.DataFrame()
        ...
        logger.error(f"Error fetching data for {symbol} from Binance: {e}", exc_info=True)
        return pd.DataFrame()


def get_or_create_asset(session, symbol_str: str, asset_type_str: str = "Unknown", dry_run: bool = False) -> Asset | None:
    """
    ...
    if not dry_run and session is None:
        logger.error("Database session is None in get_or_create_asset for non-dry_run. Cannot proceed.")
        return None

    asset: Asset | None = None
    if session: # Only query if session exists (i.e., not dry_run or dry_run with an actual session)
        try:
    ...
    if not asset:
        logger.info(f"Asset with symbol '{symbol_str}' not found. Attempting to create new asset.")
        # Create a temporary Asset object for all cases first
        temp_asset = Asset(symbol=symbol_str, name=symbol_str, asset_type=asset_type_str)

        if dry_run:
            logger.info(f"[DRY RUN] Would create asset '{symbol_str}' (type: {asset_type_str}).")
            ...
            try:
                session.commit()
                logger.info(f"Asset '{symbol_str}' created successfully with ID {temp_asset.id}.")
                session.refresh(temp_asset) # Ensure we get the ID and other DB defaults
                return temp_asset
            except IntegrityError:
                session.rollback()
                logger.warning(f"Integrity error creating asset {symbol_str}, likely created concurrently. Re-fetching.")
                try:
                    asset = session.query(Asset).filter(Asset.symbol == symbol_str).first()
                    if asset:
                        logger.info(f"Successfully re-fetched asset '{symbol_str}' after integrity error.")
                        return asset
                    else:
                        logger.error(f"Failed to re-fetch asset '{symbol_str}' after integrity error.")
                        return None
                except Exception as e_fetch:
                    logger.error(f"Error re-fetching asset {symbol_str} after integrity error: {e_fetch}", exc_info=True)
                    return None
            except Exception as e:
                session.rollback()
...
def save_data_to_db(session, data: pd.DataFrame, asset_obj: Asset, source_name: str, dry_run: bool = False):
    """
    Saves OHLCV data to the PriceData table using a bulk insert approach.
    Skips entries if a record with the same asset_id, timestamp, and source already exists.
    """
    if asset_obj is None or (not dry_run and (asset_obj.id is None or asset_obj.id == -1)):
        logger.error(f"Invalid asset object or ID for '{getattr(asset_obj, 'symbol', 'N/A')}'. Cannot save price data.")
        return

    if data.empty:
        logger.info(f"No data to save for asset '{asset_obj.symbol}' from {source_name}.")
        return
    ...
    if dry_run:
        logger.info(f"[DRY RUN] Would process {len(data)} data points for asset '{asset_obj.symbol}'.")
        # Log a few examples
        for _, row in data.head(3).iterrows():
            logger.info(f"[DRY RUN] Would add PriceData: Asset Symbol {asset_obj.symbol}, Timestamp {row[ts_column]}, "
                        f"O={row['Open']:.2f}, H={row['High']:.2f}, L={row['Low']:.2f}, C={row['Close']:.2f}, V={row['Volume']:.0f}, Source {source_name}")
        return

    # Fetch existing timestamps for the given asset and source to avoid duplicates
    existing_timestamps = {
        r[0] for r in session.query(PriceData.timestamp).filter(
            PriceData.asset_id == asset_obj.id,
            PriceData.source == source_name,
            PriceData.timestamp.in_(data[ts_column].tolist())
        )
    }

    # Filter out data that already exists
    data_to_insert_df = data[~data[ts_column].isin(existing_timestamps)]
    skipped_count = len(data) - len(data_to_insert_df)

    if data_to_insert_df.empty:
        logger.info(f"All {skipped_count} price data points for '{asset_obj.symbol}' from {source_name} already exist. Nothing new to save.")
        return
    ...
    try:
        session.bulk_insert_mappings(PriceData, data_to_insert)
        session.commit()
        logger.info(f"Successfully saved {len(data_to_insert)} new price data points for asset '{asset_obj.symbol}' from {source_name}.")
        if skipped_count > 0:
            logger.info(f"Skipped {skipped_count} existing data points.")
    except Exception as e:
        session.rollback()
        logger.error(f"Error bulk saving price data for asset '{asset_obj.symbol}': {e}", exc_info=True)
...
def main():
    ...
    parser.add_argument("--exchange", type=str, required=True, choices=['yahoo', 'binance'],
    ...
    if not logging.getLogger().hasHandlers(): # Setup basicConfig only if no handlers are present
    ...
    if args.start_date and args.end_date:
        try:
            ...
            parser.error("Invalid date format for --start_date or --end_date. Use YYYY-MM-DD.")
        if args.period:
            logger.warning("Both date range (--start_date, --end_date) and --period are specified. Date range will take precedence.")
            args.period = None
    elif args.period is None: # No dates and no period
        args.period = "1y" # Default period
        logger.info(f"No start/end date or period specified, defaulting to period='{args.period}'.")
    ...
    if not args.dry_run:
        try:
            ...
            if db_session is None: # Should not happen if SessionLocal is correctly configured
    ...
    try:
        for symbol_arg in args.symbols:
            ...
            if args.exchange == 'yahoo':
                if all(c.isalpha() or c == '.' for c in symbol_arg) and symbol_arg.isupper(): # Basic check for stock tickers like "AAPL", "BRK.A"
                    asset_type = "Stock"
            ...
            asset_object = get_or_create_asset(db_session, symbol_arg,
            ...
            if args.exchange == 'yahoo':
                ...
                ohlcv_data = fetch_yfinance_data(symbol_arg, args.start_date, args.end_date, args.period, args.interval)
            elif args.exchange == 'binance':
                source_name_str = "Binance"
                ohlcv_data = fetch_binance_data(symbol_arg, args.start_date, args.end_date, args.period, args.interval)

            if not ohlcv_data.empty:
                save_data_to_db(db_session, ohlcv_data, asset_object, source_name_str, dry_run=args.dry_run)
            else:
                logger.info(f"No data fetched for {asset_identifier_log} from {source_name_str}. Nothing to save.")

...
try:
    ...
    from ai_trader.models import OrderSide  # Enums
...
def get_random_decimal(min_val, max_val, d_places=2):
    ...
    return Decimal(random.uniform(min_val, max_val)).quantize(
        Decimal("1e-{}".format(d_places))
    )
...
def seed_trades(session, num_trades: int = 50):
    ...
    logger.info(
        f"Starting to seed {num_trades} trades (each with an associated order)..."
    )
    ...
    if not users:
        logger.error(
            "No users found. Please run seed_users.py first. Cannot create trades."
        )
        return
    if not assets:
        logger.error(
            "No assets found. Please run seed_assets.py first. Cannot create trades."
        )
        return
    # Strategies are optional, so we can proceed without them, but log a warning.
    if not strategies:
        logger.warning(
            "No strategies found. Trades will be created without strategy associations."
        )
    ...
    for _ in range(num_trades):
        ...
        order_quantity = get_random_decimal(
            1, 1000, d_places=min(8, random.randint(0, 4))
        )  # Asset quantity

        # Simulate price based on asset type (very rough simulation)
        if selected_asset.asset_type == "CRYPTO":
            order_price = get_random_decimal(
                10, 70000, d_places=min(8, random.randint(2, 6))
            )
        ...
        new_order = Order(
            ...
            price=(
                order_price if random.choice([True, False]) else None
            ),  # Price may be null for MARKET orders until filled
            ...
            + timedelta(seconds=random.randint(1, 300)),  # Simulate fill time
        )
        ...
        trade_type = (
            TradeType.BUY if new_order.order_side == OrderSide.BUY else TradeType.SELL
        )
        ...
        new_trade = Trade(
            ...
            commission_asset=(
                selected_asset.symbol if random.random() < 0.1 else "USD"
            ),  # Example
        ...
        logger.debug(
            f"Prepared Order ID {new_order.id} and Trade ID {new_trade.id} for User {selected_user.username}, Asset {selected_asset.symbol}"
        )

    try:
        session.commit()
        logger.info(
            f"Successfully seeded {orders_created_count} orders and {trades_created_count} trades."
        )
    except Exception as e:
        session.rollback()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Seed trades (and their associated orders) into the database."
    )
    parser.add_argument(
        "--num_trades", type=int, default=50, help="Number of mock trades to create."
    )

...
def seed_assets(session, num_assets: int = 10):
    ...
    while assets_created_count < num_assets:
        ...
        symbol_prefix = random.choice(
            ["FC", "TK", "ST", "CX", "FX"]
        )  # FC=FakeCoin, TK=Ticker, ST=Stock
        symbol_suffix = "".join(
            fake.random_letters(length=random.randint(2, 3))
        ).upper()
        ...
        while symbol in existing_symbols and loop_guard < 100:
            symbol_suffix = "".join(
                fake.random_letters(length=random.randint(2, 4))
            ).upper()
            symbol = f"{symbol_prefix}{symbol_suffix}"
            loop_guard += 1
        if (
            symbol in existing_symbols
        ):  # If still colliding after attempts, skip or error
            logger.warning(
                f"Could not generate a unique symbol after {loop_guard} attempts for a Faker asset. Skipping one asset."
            )
            num_assets -= 1  # reduce target as we skip one
            if num_assets <= assets_created_count:
                break
            continue
        ...
        name = (
            ...
            if asset_type == "STOCK"
                ...
                f"{fake.word().capitalize()} Coin"
                if asset_type == "CRYPTO"
                else (
                    f"{fake.currency_code()}/{fake.currency_code()}"
                    if asset_type == "FOREX"
                    else f"{fake.word().capitalize()} {asset_type.lower().capitalize()}"
                )
            )
        )

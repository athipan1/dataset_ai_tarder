...
from sqlalchemy.orm import \
    Session as SQLAlchemySession  # Renamed to avoid conflict
from sqlalchemy.orm import sessionmaker

from ai_trader.core.config import settings
from ai_trader.db.base import Base
from ai_trader.models.trade import (Trade,  # Assuming TradeType is in trade.py
                                    TradeType)
...
engine = create_engine(
    settings.DATABASE_URL, pool_recycle=3600, echo=False
...
def check_archived_trades_table_exists(session: SQLAlchemySession) -> bool:
    ...
    if ARCHIVED_TRADES_TABLE_NAME in inspector.get_table_names():
        ...
        logger.warning(
            f"Table '{ARCHIVED_TRADES_TABLE_NAME}' does not exist. Archiving to DB table will be skipped."
        )
        logger.warning("Please create this table using an Alembic migration.")
        return False


def archive_trades_to_db_table(
    session: SQLAlchemySession, trades_to_archive: list[Trade]
) -> int:
    """
    ...
    for trade in trades_to_archive:
        try:
            ...
            logger.warning(
                f"Integrity error archiving trade ID {trade.id} (possibly already archived): {ie}"
            )
        except Exception as e:
            logger.error(
                f"Error inserting trade ID {trade.id} into {ARCHIVED_TRADES_TABLE_NAME}: {e}"
            )
            raise

    if archived_count > 0:
        logger.info(
            f"Successfully prepared {archived_count} trades for archiving to table '{ARCHIVED_TRADES_TABLE_NAME}'."
        )
...
def archive_and_delete_old_trades(
    ...
    if archive_to_db:
        ...
        if not can_archive_to_db:
            logger.warning(
                f"Cannot archive to DB table '{ARCHIVED_TRADES_TABLE_NAME}' as it does not exist or is not accessible."
            )
    ...
    while running:
        ...
        while attempt < max_retries:
            try:
                ...
                trade_query = (
                    Trade.query_with_deleted(session)
                ...
                logger.info(
                    f"Fetched {len(trades_in_batch)} trades for current batch (Attempt {attempt + 1})."
                )

                archived_in_batch = 0
                if archive_to_db and can_archive_to_db and not dry_run:
                    archived_in_batch = archive_trades_to_db_table(
                        session, trades_in_batch
                    )
                elif archive_to_db and can_archive_to_db and dry_run:
                    logger.info(
                        f"[DRY RUN] Would attempt to archive {len(trades_in_batch)} trades to DB table."
                    )
                ...
                if not dry_run:
                    ...
                    deleted_in_batch = len(trades_in_batch)
                    logger.info(
                        f"[DRY RUN] Would delete {deleted_in_batch} trades from the original table."
                    )
                ...
                logger.warning(
                    f"OperationalError (e.g., lock timeout) on attempt {attempt + 1}/{max_retries}: {oe}"
                )
                if not dry_run:
                    session.rollback()
                attempt += 1
                if attempt < max_retries:
                    ...
                    logger.error(
                        "Max retries reached for the current batch due to OperationalError. Skipping this batch."
                    )
                    running = False
                    break
            except Exception as e:
    ...
    logger.info(
        f"Trade archiving and deletion finished. Total Archived: {total_archived_count}, Total Deleted: {total_deleted_count}."
    )


def main():
    parser = argparse.ArgumentParser(
        description="Archive and optionally delete old trade data from the 'trades' table."
    )
    parser.add_argument(
        "--days",
        type=int,

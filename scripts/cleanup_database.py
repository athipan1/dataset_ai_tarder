...
def archive_data_to_csv(
    ...
    if isinstance(first_item_timestamp_val, datetime):
        ...
        logger.warning(
            f"Timestamp for filename not found or not datetime for {model_name}, using current time."
        )
    ...
    try:
        with open(filename, "w", newline="", encoding="utf-8") as csvfile:
            writer = csv.DictWriter(
                csvfile, fieldnames=table_columns, extrasaction="ignore"
            )
            writer.writeheader()
        ...
        logger.info(
            f"Archived {len(data_to_archive)} rows from {model_name} to {filename}"
        )
...
def cleanup_table(
    ...
    logger.info(
        f"Processing cleanup for table '{table_name}' for data older than {cutoff_date} ({older_than_days} days ago). Archive: {archive}, Dry Run: {dry_run}"
    )

    if not Base.metadata.tables.get(table_name):
        logger.warning(
            f"Table '{table_name}' not found in SQLAlchemy metadata. Skipping cleanup for this table."
        )
        return
    ...
    try:
        ...
        if table_meta is None:
            logger.error(
                f"Could not get metadata for table {table_name} after initial check."
            )
            return
        ...
        if archive:
            ...
            if orm_model_class and hasattr(orm_model_class, timestamp_column):
                ...
                if hasattr(orm_model_class, "query_with_deleted"):
                    logger.info(
                        f"Using {orm_model_class.__name__}.query_with_deleted() for archival query."
                    )
                    data_to_archive = (
                        orm_model_class.query_with_deleted(session)
                        .filter(
                            getattr(orm_model_class, timestamp_column) < cutoff_date
                        )
                        .all()
                    )
                else:
                    logger.info(
                        f"Using session.query({orm_model_class.__name__}) for archival query (model does not have query_with_deleted)."
                    )
                    data_to_archive = (
                        session.query(orm_model_class)
                        .filter(
                            getattr(orm_model_class, timestamp_column) < cutoff_date
                        )
                        .all()
                ...
                stmt_select = text(
                    f"SELECT * FROM {table_name} WHERE {timestamp_column} < :cutoff_date"
                )
                result_proxy = session.execute(
                    stmt_select, {"cutoff_date": cutoff_date}
                )
                ...
                for row_tuple in result_proxy:  # Iterate over RowProxy directly
                    ...
                    for col_name, val in zip(
                        result_proxy.keys(), row_tuple
                    ):  # Use keys() from result_proxy
            ...
            if data_to_archive:
                if not dry_run:
                    ...
                    logger.info(
                        f"[DRY RUN] Would archive {len(data_to_archive)} rows from {table_name}."
                    )
            else:
                logger.info(
                    f"No data found older than specified date to archive for {table_name}."
                )

        if not dry_run:
            logger.info(f"Deleting data from {table_name}...")
            stmt_delete = text(
                f"DELETE FROM {table_name} WHERE {timestamp_column} < :cutoff_date"
            )
            result = session.execute(stmt_delete, {"cutoff_date": cutoff_date})
            deleted_count = result.rowcount
            session.commit()
            logger.info(
                f"Successfully deleted {deleted_count} old rows from {table_name}."
            )
        else:
            stmt_count = text(
                f"SELECT COUNT(*) FROM {table_name} WHERE {timestamp_column} < :cutoff_date"
            )
            count_result = session.execute(
                stmt_count, {"cutoff_date": cutoff_date}
            ).scalar_one()
            logger.info(
                f"[DRY RUN] Would delete {count_result} rows from {table_name}."
            )
...
def main():
    ...
    logger.info(
        f"Starting database cleanup: entity='{args.entity}', days={args.days}, archive={args.archive}, dry_run={args.dry_run}"
    )
    ...
    try:
        if args.entity == "all":
            ...
            logger.error(
                f"Unknown entity: {args.entity}. Please choose from {list(entities_to_process.keys())} or 'all'."
            )

...
def upgrade() -> None:
    ...
    with op.batch_alter_table('strategies', schema=None) as batch_op:
        ...
        batch_op.create_foreign_key('fk_strategy_user_cascade', 'users', ['user_id'], ['id'], ondelete='CASCADE')
    ...
    with op.batch_alter_table('orders', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_order_user_set_null', 'users', ['user_id'], ['id'], ondelete='SET NULL')
        batch_op.create_foreign_key('fk_order_asset_cascade', 'assets', ['asset_id'], ['id'], ondelete='CASCADE')
        batch_op.create_foreign_key('fk_order_strategy_set_null', 'strategies', ['strategy_id'], ['id'], ondelete='SET NULL')
        # signal_id FK in orders is fine as is, no ondelete specified in plan for it.

    # Trade.user_id, Trade.order_id
    with op.batch_alter_table('trades', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_trade_user_cascade', 'users', ['user_id'], ['id'], ondelete='CASCADE')
        batch_op.create_foreign_key('fk_trade_order_cascade', 'orders', ['order_id'], ['id'], ondelete='CASCADE')

    # TradeAnalytics.user_id, TradeAnalytics.strategy_id
    with op.batch_alter_table('trade_analytics', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_tradeanalytics_user_cascade', 'users', ['user_id'], ['id'], ondelete='CASCADE')
        batch_op.create_foreign_key('fk_tradeanalytics_strategy_cascade', 'strategies', ['strategy_id'], ['id'], ondelete='CASCADE')

    # UserBehaviorLog.user_id
    with op.batch_alter_table('user_behavior_logs', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_userbehaviorlog_user_cascade', 'users', ['user_id'], ['id'], ondelete='CASCADE')

    # Signal.asset_id, Signal.strategy_id
    with op.batch_alter_table('signals', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_signal_asset_cascade', 'assets', ['asset_id'], ['id'], ondelete='CASCADE')
        batch_op.create_foreign_key('fk_signal_strategy_cascade', 'strategies', ['strategy_id'], ['id'], ondelete='CASCADE')

    # PriceData.asset_id
    with op.batch_alter_table('price_data', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_pricedata_asset_cascade', 'assets', ['asset_id'], ['id'], ondelete='CASCADE')

    # BacktestResult.strategy_id
    with op.batch_alter_table('backtest_results', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_backtestresult_strategy_cascade', 'strategies', ['strategy_id'], ['id'], ondelete='CASCADE')

    # Composite Indexes
    op.create_index('ix_order_status_user_created', 'orders', ['status', 'user_id', 'created_at'], unique=False)
    ...
    try:
        ...
        print(f"Info: Could not drop index 'ix_trade_user_id_symbol_timestamp' (may not exist): {e}")
    except Exception as e:
        print(f"Warning: An unexpected error occurred while dropping index 'ix_trade_user_id_symbol_timestamp': {e}")
    op.create_index('ix_trade_user_symbol_timestamp_type', 'trades', ['user_id', 'symbol', 'timestamp', 'trade_type'], unique=False)


    # Partial Index (PostgreSQL specific)
    bind = op.get_bind()
    if bind.dialect.name == 'postgresql':
        op.create_index('idx_open_orders_user_id', 'orders', ['user_id'], unique=False, postgresql_where=sa.text("status = 'OPEN'"))
...
def downgrade() -> None:
    ...
    bind = op.get_bind()
    if bind.dialect.name == 'postgresql':
        op.drop_index('idx_open_orders_user_id', table_name='orders', postgresql_where=sa.text("status = 'OPEN'"))
    ...
    with op.batch_alter_table('trade_analytics', schema=None) as batch_op:
        ...
        batch_op.drop_constraint('fk_tradeanalytics_strategy_cascade', type_='foreignkey')
    ...
    with op.batch_alter_table('backtest_results', schema=None) as batch_op:
        batch_op.drop_constraint('fk_backtestresult_strategy_cascade', type_='foreignkey')

...
class TestDataPipeline(unittest.TestCase):
    ...
    def test_calculate_features_rsi(self):
        ...
        self.assertFalse(
            features_df["rsi_14"].iloc[14:].isna().any(),
        ...
        self.assertIn(
            "timestamp", features_df.columns, "Timestamp column should be preserved."
        )
    ...
    def test_calculate_features_all_present(self):
        ...
        for col in expected_feature_model_columns:
            self.assertIn(
                col,
                features_df.columns,
                f"Model column {col} is missing from calculated features.",
            )
    ...
    def test_get_price_data(self, mock_session_local):
        ...
        mock_price_data_query.all.return_value = (
            mock_records  # This is what all() should return
        )
        ...
        df = get_price_data(
            mock_session, asset_id=1, start_date="2023-01-01", end_date="2023-01-02"
        )
    ...
    def test_save_features_to_db_dry_run(self):
        ...
        features_data = {
            ...
            ),  # After initial NaN period for most indicators
            "rsi_14": [50.0, 52.0],
            "sma_20": [13.0, 13.5],
            "ema_20": [13.2, 13.3],
            # ... other features ...
    ...
    @patch("ai_trader.data_pipeline.pg_insert")
    def test_save_features_to_db_actual_save(self, mock_pg_insert):
        ...
        features_data = {
            ...
            "ema_50": [12.2, 12.3],
            "macd_line": [0.5, 0.6],
            "macd_signal": [0.4, 0.45],
            "macd_hist": [0.1, 0.15],
            "atr_14": [1.0, 1.1],
            "bb_upperband": [15.0, 15.5],
            "bb_middleband": [13.0, 13.5],
            "bb_lowerband": [11.0, 11.5],
        }
        sample_features_df = pd.DataFrame(features_data)

        save_features_to_db(mock_session, asset_id, sample_features_df, dry_run=False)

        mock_pg_insert.assert_called_once_with(Features)

...
def test_user_behavior_log_relationships(db_session, test_user):
    ...
    retrieved_user = (
            db_session.query(User).filter_by(id=test_user.id, is_deleted=False).one()
    )
...
def test_trade_analytics_relationships(db_session, test_user, test_strategy):
    ...
    retrieved_strategy = (
        db_session.query(Strategy).filter_by(id=test_strategy.id, is_deleted=False).one()
    )
...
def test_user_deletion_cascades_and_set_null(
    ...
    deleted_user = (
        db_session.query(User).filter(User.id == user_id).one_or_none()
    )
    ...
    deleted_order = (
        db_session.query(Order).filter(Order.id == order_id).one_or_none()
    )
    ...
    deleted_trade = (
        db_session.query(Trade).filter(Trade.id == trade_id).one_or_none()
    )
    ...
    deleted_analytics = (
        db_session.query(TradeAnalytics)
    ...
    deleted_log = (
        db_session.query(UserBehaviorLog)
...
def test_soft_delete_mixin_behavior(db_session, test_user: User):
    ...
    retrieved_user_std = (
        db_session.query(User).filter_by(id=user_id, is_deleted=False).one_or_none()
    )
    ...
    retrieved_user_all = (
        db_session.query(User).filter_by(id=user_id).one_or_none()
    )
    ...
    test_user.soft_delete()  # Call again
    db_session.commit()

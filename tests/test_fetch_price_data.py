...
class TestFetchPriceData(unittest.TestCase):
    ...
    def test_fetch_yfinance_data_success(self, mock_ticker_constructor):
        ...
        }, index=pd.to_datetime([datetime(2023, 1, 1, 0, 0, 0, tzinfo=timezone.utc),
        ...
        mock_ticker_instance.history.return_value = raw_yf_output # Return with Datetime as index
        mock_ticker_constructor.return_value = mock_ticker_instance

        symbol = "TEST.SA"
        start_date = "2023-01-01"
        end_date = "2023-01-03" # yfinance end is exclusive for daily, so script adds 1 day

        # Expected end_date for yf.Ticker().history call
        expected_yf_end_date = (datetime.strptime(end_date, "%Y-%m-%d") + pd.Timedelta(days=1)).strftime("%Y-%m-%d")


        df = fetch_yfinance_data(symbol, start_date=start_date, end_date=end_date, interval="1d")
        ...
        self.assertListEqual(list(df.columns), ['Datetime', 'Open', 'High', 'Low', 'Close', 'Volume'])
    ...
    def test_fetch_yfinance_data_empty(self, mock_ticker_constructor):
        ...
        df = fetch_yfinance_data("FAIL.SA", start_date="2023-01-01", end_date="2023-01-03", interval="1d")
    ...
    def test_fetch_yfinance_data_api_error(self, mock_ticker_constructor):
        ...
        df = fetch_yfinance_data("ERROR.SA", start_date="2023-01-01", end_date="2023-01-03", interval="1d")
        self.assertTrue(df.empty)

    def test_get_or_create_asset_new(self):
        """Test creating a new asset."""
        mock_session = MagicMock()
        mock_session.query(Asset).filter(Asset.symbol == "NEWCO").first.return_value = None # Simulate asset not found
    ...
    def test_get_or_create_asset_existing(self):
        ...
        mock_existing_asset = Asset(id=1, symbol="OLDCO", name="OLDCO", asset_type="Stock")
        mock_session = MagicMock()
        mock_session.query(Asset).filter(Asset.symbol == "OLDCO").first.return_value = mock_existing_asset
    ...
    def test_get_or_create_asset_dry_run_new(self):
        ...
        mock_session = MagicMock()
        mock_session.query(Asset).filter(Asset.symbol == "DRYNEW").first.return_value = None
    ...
    def test_save_data_to_db(self):
        ...
        mock_session.query(PriceData).filter_by(asset_id=1, timestamp=pd.to_datetime("2023-01-01"), source="TestLSource").first.return_value = None
        mock_session.query(PriceData).filter_by(asset_id=1, timestamp=pd.to_datetime("2023-01-02"), source="TestSource").first.return_value = None
        ...
        data_to_save = pd.DataFrame({
            ...
            'Datetime': pd.to_datetime(["2023-01-01", "2023-01-02"]) # Naive, as expected after fetch processing
        })

        save_data_to_db(mock_session, data_to_save, test_asset, "TestSource", dry_run=False)
    ...
    def test_save_data_to_db_dry_run(self):
        ...
        mock_session = MagicMock() # Session won't be used for DB ops in dry_run for PriceData
        ...
        save_data_to_db(mock_session, data_to_save, test_asset_dry_created, "TestSourceDry", dry_run=True)
    ...
    def test_save_data_to_db_skip_existing(self):
        ...
        mock_session = MagicMock()
        existing_pd_entry = PriceData(asset_id=1, timestamp=pd.to_datetime("2023-01-01"), source="TestSource", open=1,high=1,low=1,close=1,volume=1)

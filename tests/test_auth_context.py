...
user_context_var: ContextVar[CurrentUser | None] = ContextVar("user_context_var", default=None)
...
def mock_user_valid():
    ...
    return CurrentUser(user_id=user.id, username=user.username, is_superuser=user.is_superuser)
...
def test_auth_context_manager_sets_and_resets_user(mock_user_valid):
    ...
    assert user_context_var.get() is None  # Should be None initially or from fixture reset
    ...
    assert user_context_var.get() is None  # Should be reset to None after exiting context
...
def test_current_user_model_creation():
    ...
    user_data_super = {"user_id": 11, "username": "pydantic_super", "is_superuser": True}

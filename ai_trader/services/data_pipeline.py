...
def calculate_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    }, inplace=True, errors='ignore') # ignore errors if already lowercase
    ...
    df.ta.bbands(length=20, std=2, append=True,
                 col_names=('bb_lowerband', 'bb_middleband', 'bb_upperband', 'bb_bandwidth', 'bb_percent'))
    ...
    if 'timestamp' in df.columns:
        ...
        logger.error("Timestamp column is missing from DataFrame for feature calculation.")
        return pd.DataFrame()
...
def get_price_data(db: Session, asset_id: int, start_date: Optional[str] = None, end_date: Optional[str] = None) -> pd.DataFrame:
    """
    Retrieves PriceData for a given asset_id and optional date range.
    """
    logger.info(f"Fetching price data for asset_id: {asset_id} (start: {start_date}, end: {end_date})")
    ...
    if not price_data_records:
        logger.warning(f"No price data found for asset_id: {asset_id} in the given range.")
        return pd.DataFrame()
...
def save_features_to_db(db: Session, asset_id: int, features_df: pd.DataFrame, dry_run: bool = False):
    """
    Saves calculated features to the Features table using a bulk insert approach.
    """
    if features_df.empty:
        logger.info("No features to save.")
        return

    logger.info(f"Preparing to save {len(features_df)} feature sets for asset_id: {asset_id}.")

    # Get existing timestamps to avoid duplicates
    existing_timestamps = {
        r[0] for r in db.query(Features.timestamp).filter(
            Features.asset_id == asset_id,
            Features.timestamp.in_(features_df['timestamp'].tolist())
        )
    }

    # Filter out existing features
    features_to_insert_df = features_df[~features_df['timestamp'].isin(existing_timestamps)]
    skipped_count = len(features_df) - len(features_to_insert_df)

    if features_to_insert_df.empty:
        logger.info(f"All {skipped_count} features already exist in the database. Nothing new to save.")
        return

    # Prepare data for bulk insert
    features_to_insert = []
    for _, row in features_to_insert_df.iterrows():
        feature_data = {col: None if pd.isna(row[col]) else row[col] for col in row.index}
        feature_data['asset_id'] = asset_id
        features_to_insert.append(feature_data)

    if dry_run:
        logger.info(f"[DRY RUN] Would insert {len(features_to_insert)} new feature sets.")
        logger.info(f"[DRY RUN] Skipped {skipped_count} existing feature sets.")
        return

    try:
        db.bulk_insert_mappings(Features, features_to_insert)
        db.commit()
        logger.info(f"Successfully saved {len(features_to_insert)} new feature sets for asset_id: {asset_id}.")
        logger.info(f"Skipped {skipped_count} existing feature sets.")
    except Exception as e:
        db.rollback()
        logger.error(f"Error bulk saving features for asset_id {asset_id}: {e}", exc_info=True)


def main():
    parser = argparse.ArgumentParser(description="Calculate technical features from PriceData and store them.")
    ...
    try:
        ...
        if not asset:
            logger.error(f"Asset with symbol '{args.symbol}' not found. Please ensure data is fetched first using fetch_price_data.py.")
            return
        ...
        if price_df.empty:
            logger.warning(f"No price data retrieved for {asset.symbol}. Cannot calculate features.")
            return

        # Calculate Features
        features_df = calculate_features(price_df.copy()) # Pass a copy to avoid SettingWithCopyWarning
        if features_df.empty:
            logger.warning(f"No features calculated for {asset.symbol}. Nothing to save.")
            return
        ...
        logger.critical(f"A critical error occurred in the feature engineering pipeline: {e}", exc_info=True)
    finally:
        if db:
            db.close()

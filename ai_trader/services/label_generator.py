...
def generate_labels(features_df: pd.DataFrame) -> pd.DataFrame:
    """
    Generates BUY/SELL/HOLD labels based on EMA20 and EMA50 using vectorized operations.
    ...
    choices = [SignalType.BUY, SignalType.SELL]
    features_df['signal_type'] = pd.Series(
        pd.NA, index=features_df.index
    ).mask(conditions[0], choices[0]).mask(conditions[1], choices[1]).fillna(SignalType.HOLD)

    logger.info(f"Generated {len(features_df)} labels. Counts: {features_df['signal_type'].value_counts().to_dict()}")
    return features_df


def save_labels_to_db(db: Session, asset_id: int, strategy_id: int, labels_df: pd.DataFrame, dry_run: bool = False):
    """
    Saves generated labels to the Signal table using a bulk insert approach.
    """
    ...
    logger.info(f"Preparing to save {len(labels_df)} labels for asset_id: {asset_id}, strategy_id: {strategy_id}.")

    # Get existing timestamps to avoid duplicates for this asset and strategy
    existing_timestamps = {
        r[0] for r in db.query(Signal.timestamp).filter(
            Signal.asset_id == asset_id,
            Signal.strategy_id == strategy_id,
            Signal.timestamp.in_(labels_df['timestamp'].tolist())
        )
    }

    # Filter out existing signals
    labels_to_insert_df = labels_df[~labels_df['timestamp'].isin(existing_timestamps)]
    skipped_count = len(labels_df) - len(labels_to_insert_df)

    if labels_to_insert_df.empty:
        logger.info(f"All {skipped_count} signals already exist for this strategy. Nothing new to save.")
        return

    # Prepare data for bulk insert
    signals_to_insert = []
    for _, row in labels_to_insert_df.iterrows():
        signal_data = {
            "asset_id": asset_id,
            "strategy_id": strategy_id,
            "timestamp": row['timestamp'],
            "signal_type": row['signal_type'],
            "price_at_signal": None if pd.isna(row.get('price_at_signal')) else row.get('price_at_signal')
        }
        signals_to_insert.append(signal_data)

    if dry_run:
        logger.info(f"[DRY RUN] Would insert {len(signals_to_insert)} new signals.")
        logger.info(f"[DRY RUN] Skipped {skipped_count} existing signals.")
        return

    try:
        db.bulk_insert_mappings(Signal, signals_to_insert)
        db.commit()
        logger.info(f"Successfully saved {len(signals_to_insert)} new signals for asset_id: {asset_id}, strategy_id: {strategy_id}.")
        logger.info(f"Skipped {skipped_count} existing signals.")
    except Exception as e:
        db.rollback()
        logger.error(f"Error bulk saving signals: {e}", exc_info=True)

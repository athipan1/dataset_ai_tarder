...
class Asset(Base):
    ...
    )  # e.g., BTCUSD, ETHUSD, EURUSD, AAPL
    name = Column(
        String, nullable=True
    )  # e.g., Bitcoin, Ethereum, Euro/US Dollar, Apple Inc.
    asset_type = Column(DBEnum(AssetType), default=AssetType.UNKNOWN, index=True)
    exchange = Column(
        String, nullable=True, index=True
    )  # e.g., NASDAQ, NYSE, BINANCE, FTX
    ...
    price_data = relationship(
        "PriceData", back_populates="asset", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        Index(
            "ix_asset_symbol", "symbol", unique=True
        ),  # unique=True was already on column

...
class Asset(Base):
    ...
    orders = relationship("Order", back_populates="asset") # Don't cascade delete orders if asset is deleted, handle manually or set NULL.
    ...
    def __repr__(self):
        return f"<Asset(id={self.id}, symbol='{self.symbol}', type='{self.asset_type.value if self.asset_type else None}')>"

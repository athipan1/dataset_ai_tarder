from sqlalchemy import Column, Integer, DateTime, Numeric, ForeignKey, Index, Text, JSON, String
...
class BacktestResult(Base):
    ...
    __table_args__ = (
        Index("ix_backtest_result_strategy_id_created_at", "strategy_id", "created_at", unique=False), # A strategy can have multiple backtests
        # Index("ix_backtest_result_user_id_created_at", "user_id", "created_at"), # if user_id is added
    )

    def __repr__(self):
        return f"<BacktestResult(id={self.id}, strategy_id={self.strategy_id}, name='{self.name}', pnl_pct={self.pnl_percentage})>"

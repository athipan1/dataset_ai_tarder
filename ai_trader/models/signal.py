import enum
from sqlalchemy import Column, Integer, DateTime, Numeric, ForeignKey, Index, Enum as DBEnum, Text
...
class Signal(Base):
    ...
    __table_args__ = (
        Index("ix_signal_asset_strategy_timestamp", "asset_id", "strategy_id", "timestamp", unique=False), # Signals might not be unique for this combo always
    ...
    def __repr__(self):
        return f"<Signal(id={self.id}, asset_id={self.asset_id}, strategy_id={self.strategy_id}, type='{self.signal_type.value}', time='{self.timestamp}')>"

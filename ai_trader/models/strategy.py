...
class Strategy(Base):
    ...
    parameters = Column(JSON, nullable=True) # From original root models.py. For PG, use sqlalchemy.dialects.postgresql.JSONB
    ...
    __table_args__ = (
        Index("ix_strategy_user_id_name", "user_id", "name", unique=True), # A user shouldn't have two strategies with the same name

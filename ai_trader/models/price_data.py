...
class PriceData(Base):
    ...
    )  # Cascade delete if asset is deleted
    ...
    __table_args__ = (
        Index(
            "ix_price_data_asset_id_timestamp_source",
            "asset_id",
            "timestamp",
            "source",
            unique=True,
        ),  # Most specific unique constraint for asset/time/source
        Index("ix_price_data_timestamp", "timestamp"),
        # asset_id is already indexed by ForeignKey for many DBs,
        # but explicit index is fine if needed for other types of queries.
        # Index("ix_price_data_asset_id", "asset_id"),
    )

    def __repr__(self):
        return (
            f"<PriceData(id={self.id}, asset_id={self.asset_id}, "

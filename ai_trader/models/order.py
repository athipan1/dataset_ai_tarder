import enum
from sqlalchemy import Column, Integer, DateTime, Numeric, ForeignKey, Index, String, Enum as DBEnum, Boolean
...
class Order(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True) # If user deleted, keep order
    asset_id = Column(Integer, ForeignKey("assets.id", ondelete="RESTRICT"), nullable=False, index=True) # Don't delete asset if orders exist
    ...
    exchange_order_id = Column(String, nullable=True, index=True, unique=False) # Exchange ID might not be unique if same ID used on different exchanges or test environments
    ...
    def __repr__(self):
        return f"<Order(id={self.id}, user_id={self.user_id}, asset_id={self.asset_id}, status='{self.status.value}')>"

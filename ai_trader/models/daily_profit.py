...
class DailyProfit(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("profit_date", "user_id", "strategy_id", name="uq_daily_profit_date_user_strategy"),
        Index("ix_daily_profit_date", "profit_date"),
    )

    def __repr__(self):
        return f"<DailyProfit(date='{self.profit_date}', user_id={self.user_id}, strategy_id={self.strategy_id}, profit={self.total_profit})>"

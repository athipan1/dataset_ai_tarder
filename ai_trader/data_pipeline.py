...
def save_features_to_db(
    ...
    logger.info(
        f"Preparing to save {len(features_df)} feature sets for asset_id: {asset_id}."
    )

    # Convert DataFrame to a list of dictionaries
    features_to_insert = features_df.to_dict(orient="records")
    for feature in features_to_insert:
        feature["asset_id"] = asset_id

    if dry_run:
        logger.info(
            f"[DRY RUN] Would insert {len(features_to_insert)} feature records."
        )
        for feature_data in features_to_insert[:5]:  # Log first 5 as a sample
            logger.info(f"[DRY RUN] Sample feature: {feature_data}")
        return

    try:
        # Use PostgreSQL's ON CONFLICT DO NOTHING to avoid duplicates
        stmt = pg_insert(Features).values(features_to_insert)
        stmt = stmt.on_conflict_do_nothing(index_elements=["asset_id", "timestamp"])
        result = db.execute(stmt)
        db.commit()
        logger.info(
            f"Successfully saved features for asset_id {asset_id}. "
            f"Inserted: {result.rowcount} new records."
        )
    except IntegrityError:
        db.rollback()
        logger.error(
            f"Integrity error during bulk insert for asset_id {asset_id}. "
            "This might happen if a race condition occurs. Retrying row-by-row is a possible fallback.",
            exc_info=True,
        )
    except Exception as e:
        db.rollback()
        logger.error(
            f"Error during bulk saving features for asset_id {asset_id}: {e}",
            exc_info=True,
        )

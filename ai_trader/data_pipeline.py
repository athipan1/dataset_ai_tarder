...
from ai_trader.models import Asset, PriceData, Features
...
def save_features_to_db(db: Session, asset_id: int, features_df: pd.DataFrame, dry_run: bool = False):
    ...
    for _, row in features_df.iterrows():
        ...
        feature_data = {col: None if pd.isna(row[col]) else row[col] for col in row.index if col != 'timestamp'}
        feature_data['asset_id'] = asset_id
        feature_data['timestamp'] = row['timestamp'] # Timestamp is from the index/column

        if dry_run:
            logger.info(f"[DRY RUN] Would save Features for asset_id {asset_id} at {row['timestamp']}: {feature_data}")
            added_count += 1
            continue

        # Check for existing entry
        try:
            existing_feature = db.query(Features).filter_by(
                asset_id=asset_id,
            ...
            logger.error(f"Error querying existing Feature for asset {asset_id} at {row['timestamp']}: {e_query}", exc_info=True)
            error_count += 1
            continue
        ...
        try:
            ...
            db.rollback()
            logger.debug(f"Integrity error (likely duplicate) for asset_id {asset_id} at {row['timestamp']}. Skipping.")
            skipped_count += 1
        except Exception as e:
            db.rollback()
            logger.error(f"Error saving feature for asset_id {asset_id} at {row['timestamp']}: {e}", exc_info=True)
            error_count += 1

    log_prefix = "[DRY RUN] " if dry_run else ""
    logger.info(f"{log_prefix}Features saving for asset_id {asset_id} complete. "

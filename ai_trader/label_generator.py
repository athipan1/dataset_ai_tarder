...
def get_or_create_strategy(
    db: Session, strategy_name: str, user_id: int, dry_run: bool = False
) -> Optional[Strategy]:
    """
    ...
    strategy = (
        db.query(Strategy)
        .filter(Strategy.name == strategy_name, Strategy.user_id == user_id)
        .first()
    )
    if not strategy:
        logger.info(
            f"Strategy '{strategy_name}' for user_id {user_id} not found. Attempting to create."
        )
        if dry_run:
            logger.info(
                f"[DRY RUN] Would create strategy '{strategy_name}' for user_id {user_id}."
            )
            # Return a temporary strategy object for dry_run flow
            temp_strategy = Strategy(
                name=strategy_name,
                user_id=user_id,
                description="Automated labeling strategy based on EMA crossover.",
            )
        ...
        if not user:
            logger.error(
                f"User with ID {user_id} not found. Cannot create strategy '{strategy_name}'."
            )
            ...
            logger.warning(
                f"Please create User ID {user_id} or ensure it exists in the database."
            )
            ...
            if user_id == DEFAULT_USER_ID:
                logger.info(
                    f"Attempting to create default user with ID {DEFAULT_USER_ID} for strategy creation."
                )
                default_user = User(
                    id=DEFAULT_USER_ID,
                    username=f"default_user_{DEFAULT_USER_ID}",
                    email=f"user{DEFAULT_USER_ID}@example.com",
                    hashed_password="!",
                )  # Dummy password
                try:
                    db.add(default_user)
                    ...
                    db.rollback()
                    logger.warning(
                        f"Default user {DEFAULT_USER_ID} might have been created concurrently. Re-fetching."
                    )
                    user = db.query(User).filter(User.id == user_id).first()
                except Exception as e_user:
                    db.rollback()
                    logger.error(
                        f"Failed to create default user {DEFAULT_USER_ID}: {e_user}"
                    )
                    return None
            if not user:  # If still no user after attempt
                logger.error(
                    f"User with ID {user_id} still not found after attempting creation. Cannot create strategy."
                )
                return None
        ...
        try:
            ...
            db.commit()
            logger.info(
                f"Strategy '{strategy_name}' created successfully with ID {strategy.id} for user_id {user_id}."
            )
            db.refresh(strategy)
        except (
            IntegrityError
        ):  # Should not happen if query().first() was accurate, but as safeguard
            db.rollback()
            logger.warning(
                f"Integrity error creating strategy {strategy_name}, it might have been created concurrently. Re-fetching."
            )
            strategy = (
                db.query(Strategy)
                .filter(Strategy.name == strategy_name, Strategy.user_id == user_id)
            ...
            if not strategy:
                logger.critical(
                    f"Failed to create or find strategy {strategy_name} after integrity error."
                )
                return None
        ...
        logger.debug(
            f"Found existing strategy '{strategy_name}' with ID {strategy.id} for user_id {user_id}."
        )
    return strategy
...
def get_features_data(
    ...
    """
    logger.info(
        f"Fetching features data for asset_id: {asset_id} (start: {start_date}, end: {end_date})"
    )
    query = (
        db.query(
            Features.timestamp,
            Features.ema_20,
            Features.ema_50,
            PriceData.close.label(
                "price_at_signal"
            ),  # Get close price from PriceData for the signal
        )
        .join(
            PriceData,
            (Features.asset_id == PriceData.asset_id)
            & (Features.timestamp == PriceData.timestamp),
        )
    ...
    if not feature_records:
        logger.warning(
            f"No features data found for asset_id: {asset_id} in the given range."
        )
        return pd.DataFrame()

    df = pd.DataFrame(
        feature_records, columns=["timestamp", "ema_20", "ema_50", "price_at_signal"]
    )
...
def generate_labels(features_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if (
        features_df.empty
        or "ema_20" not in features_df.columns
        or "ema_50" not in features_df.columns
    ):
        logger.warning(
            "Features DataFrame is empty or missing EMA columns. Cannot generate labels."
        )
        return pd.DataFrame()
    ...
    logger.info(
        f"Generated {len(labels)} labels. Counts: {features_df['signal_type'].value_counts().to_dict()}"
    )
    return features_df
...
def save_labels_to_db(
    ...
    if (
        labels_df.empty
        or "signal_type" not in labels_df.columns
        or "timestamp" not in labels_df.columns
    ):
    ...
    logger.info(
        f"Saving {len(labels_df)} labels for asset_id: {asset_id}, strategy_id: {strategy_id}."
    )
    ...
    for _, row in labels_df.iterrows():
        ...
        if not isinstance(signal_type_enum, SignalType):  # Ensure it's the enum type
            try:
                ...
                logger.error(
                    f"Invalid signal type value: {row['signal_type']}. Skipping."
                )
                error_count += 1
                continue

        price_at_signal = row.get(
            "price_at_signal"
        )  # Get from DataFrame, might be None
        ...
        try:
            ...
            logger.error(
                f"Error querying existing Signal for asset {asset_id}, strategy {strategy_id} at {row['timestamp']}: {e_query}",
                exc_info=True,
            )
            error_count += 1
            continue
        ...
        try:
            ...
            db.rollback()
            logger.debug(
                f"Integrity error (likely duplicate) for signal. Asset: {asset_id}, Strat: {strategy_id}, TS: {row['timestamp']}. Skipping."
            )
            skipped_count += 1
        except Exception as e:
            db.rollback()
            logger.error(
                f"Error saving signal for asset_id {asset_id}, strategy_id {strategy_id} at {row['timestamp']}: {e}",
                exc_info=True,
            )
            error_count += 1
    ...
    logger.info(
        f"{log_prefix}Signal saving for asset_id {asset_id}, strategy_id {strategy_id} complete. "
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate BUY/SELL/HOLD labels based on EMA crossovers and save as Signals."
    )
    parser.add_argument(
    ...
    try:
        ...
        if not asset:
            logger.error(
                f"Asset with symbol '{args.symbol}' not found. Please ensure data is fetched and features are generated first."
            )
            return
        ...
        strategy = get_or_create_strategy(
            db, args.strategy_name, args.user_id, args.dry_run
        )
        if not strategy or strategy.id is None:  # strategy.id can be -1 in dry_run
            logger.error(
                f"Could not get or create strategy '{args.strategy_name}'. Cannot proceed."
            )
            return
        ...
        if features_df.empty:
            logger.warning(
                f"No features data retrieved for {asset.symbol}. Cannot generate labels."
            )
            return
        ...
        logger.critical(
            f"A critical error occurred in the label generation pipeline: {e}",
            exc_info=True,
        )
    finally:
        if db:

...
class Settings:
    ...
    POSTGRES_PORT: Optional[str] = os.getenv("POSTGRES_PORT", "5432") # Default PostgreSQL port
    ...
    if not DATABASE_URL and all([POSTGRES_SERVER, POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_DB]):
        ...
        print(f"Warning: DATABASE_URL or PostgreSQL environment variables not fully set. Falling back to SQLite: {DATABASE_URL}")
    ...
    if SECRET_KEY == "a_very_secret_default_key_for_dev_only_CHANGE_ME":
        print("Warning: SECRET_KEY is using the default development value. This should be changed for production.")
    ...
    class Config:
        case_sensitive = True # Ensure environment variable names are treated as case-sensitive
